...
| Fresh .ds export capturing all Session 31 work. v22 (2026-08-12) is already stale: it predates "Referrals Main On Create - Master" (label resolver + Partner_ID_Stamp), the rewritten get_partner_referral_contact (lookup-based branch/org, digits-only phone), the deleted REF ID Generator / Partner Contact Upsert, and the two generators switched to Edited. Repo sync blocked until exported. | Neil | OPEN | Y | 2026-08-13 |
| Referral notification emails from Creator: SendGrid dynamic template is built (HTML tables, {{#equals}} location variants, {{#if AC_First_Name}} block). Still to do: append a send step to the end of the master workflow using a send_via_sendgrid variant that posts template_id + dynamic_template_data. NEEDED FROM NEIL: full SendGrid template ID (d-...) and the final merge-tag list exactly as saved. Zoho Forms' own notification emails cannot carry the Referral ID (it does not exist at submit time) and must be turned off. Internal recipients only (Neil + providers), so no partner-facing PHI concern yet. | cchat / Neil | OPEN | N | 2026-08-13 |

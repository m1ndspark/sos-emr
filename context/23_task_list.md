...
| Re-export the .ds and commit. v14 is stale: four new backfills (backfill_pvs_ids, resolve_referral_branch_from_text, backfill_pvs_complexity_charge, backfill_pvs_patient_full_address), six diagnostics, one new field (Patient_Full_Address), and one workflow fix (Build Patient Full Address input. prefix). Placeholder .dg files are in the repo until then. | ccode | OPEN | N | next session |

...
`pre-commit` adds one commit-scoped warning:

- **WARN** `.dg` filename without a known `On<Event>__` prefix

(The `_INDEX.md` coupling warning was removed 2026-09-25. `_INDEX.md` is frozen;
`MANIFEST.tsv` is the index and is regenerated by ds_sync, never hand-edited.)
